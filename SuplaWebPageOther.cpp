...
#ifdef SUPLA_VINDRIKTNING
  addFormHeader(webContentBuffer, String(S_GPIO_SETTINGS_FOR) + S_AIR_SENSOR_TX);
  addListGPIOBox(webContentBuffer, INPUT_AIR_SENSOR_GPIO, F("GPIO"), FUNCTION_AIR_SENSOR_TX);
  addFormHeaderEnd(webContentBuffer);
#endif
...
void handleOtherSave() {
  ...
  for (nr = 1; nr <= 3; nr++) {
    if (!WebServer->saveGPIO(INPUT_PZEM_RX, FUNCTION_PZEM_RX, nr) || !WebServer->saveGPIO(INPUT_PZEM_TX, FUNCTION_PZEM_TX, nr)) {
     ...
     if (!WebServer->saveGPIO(INPUT_AIR_SENSOR_GPIO, FUNCTION_AIR_SENSOR_TX))  {
        handleOther(6);
      return;
      }
#endif
